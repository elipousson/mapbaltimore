...
#' @param crs EPSG code for the coordinate reference system for the plot. Default is 2804. See \url{https://epsg.io/} for more information.
#'
#' @export
...
get_osm_feature <- function(area,
  ...
  # Get area bounding box with web mercator CRS
  area_bbox <- area %>%
  ...
  if (trim) {
    area_osm_sf <- sf::st_intersection(area_osm_sf, area)
  }
