...
map_area_zoning <- function(
  ...
  # Nest area data
  area_nested <-
    dplyr::nest_by(area, name, .keep = TRUE)
  ...
  area_zoning_map <- purrr::map2(
    ...
    ~
      ...
      ggplot2::ggplot() +
        ...
        ggplot2::geom_sf(data = .x, color = "white", size = 1.2, fill = NA) +
        ggplot2::geom_sf(data = .x, color = "gray20", fill = NA, linetype = 5) +
