...
- Storing large (15-25 MB) data sets as geopackage (.gpkg) files in the `inst/extdata` folder
...
This data can be accessed using the extdata parameter of the `getdata::get_location_data()` function when package is set to "mapbaltimore" as the following examples illustrate.

```{r trees}
# Get trees in the Oliver neighborhood
oliver_trees <- get_area(type = "neighborhood", area_name = "Oliver") %>%
  getdata::get_location_data(data = "trees.gpkg", package = "mapbaltimore")
...
get_area_data(area = druidhill, extdata = "vegetated_area") %>%
  ...
  maplayer::layer_mask(data = druidhill, alpha = 0.5, fill = "white", color = NA, dist = 5)
```
...
get_area(type = "neighborhood", area_name = "Federal Hill") %>%
  getdata::get_location_data(data = "edge_of_pavement", package = "mapbaltimore") %>%
  ggplot() +
...
baltimore_county %>%
  ...
  maplayer::layer_neatline(data = baltimore_county, crs = 2804) +
  guides(fill = "none") +
