...
Not all of the data for mapbaltimore is loaded when you first load the package. Large spatial datasets can take a long time to load and then slow down the whole environment after they are loaded into memory. GitHub also has a maximum file size of 100 megabytes. While some datasets are rarely updated, others are regularly updated by Baltimore City and the state of Maryland leaving the datasets included with this package potentially outdated.

mapbaltimore works around these issues for large in a few different ways:
...
- Offering other functions to access remote spatial data on city and state ArcGIS servers, the state Socrata-based state open data portal, and on Open Street Map

Both external and cached data sources are never fully loaded into memory but instead use the `wkt_filter` parameter for the `sf::st_read()` function as described in [this post on Working with Large Spatial Data in R](https://jayrobwilliams.com/posts/2020/09/spatial-sql) by Rob Williams.
...
get_area_data(area = druidhill, extdata = "vegetated_area") %>% 
  ...
  layer_area_mask(area = druidhill, color = NA, alpha = 0.5, fill = "white", dist = 5)
```
...
```{r layer_area_streets_msa}
second_district <- get_area(type = "congressional district", area_name = "2nd District")

district_map <- ggplot() +
  geom_sf(data = second_district, color = NA, fill = "springgreen", alpha = 0.2)
...
get_area_crashes(geometry = TRUE, trim = TRUE) %>% 
  ...
  geom_sf(data = charles_north, color = "black", fill = NA)
```
