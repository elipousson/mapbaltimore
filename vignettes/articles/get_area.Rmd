...
You can review the names (`name`) or identifiers (`id`) for each type of area by looking at the corresponding column in the data. Typically, the name column should also work as a label for an area and the id column is used as a unique identifier. The names require an exact match. For example, `get_baltimore_area(type = "neighborhood", area_name = "Washington Village/Pigtown")` works but `get_baltimore_area(type = "neighborhood", area_name = "Pigtown")` will return an error.
...
Finally, here is how these area adjustments work in combination with the `getdata::get_location_data()` function. By default, the data is cropped to the bounding box of the provided area:
...
```{r trim_example}
area_trees <- getdata::get_location_data(
  location = sf::st_bbox(area),
  data = "trees",
  fileext = "gpkg",
  dist = example_dist,
...
area_trees_trimmed <- getdata::get_location_data(
  location = area,
  data = "trees",
  dist = example_dist,
  trim = TRUE,
...
```{r trim_example_alt}
area_trees_trimmed_diag_ratio <- getdata::get_location_data(
  location = sfext::st_buffer_ext(area, diag_ratio = example_diag_ratio),
  data = "trees",
  package = "mapbaltimore",
  trim = TRUE
...
The `trim` parameter is also supported by the `getdata::get()` and `get_area_osm_data()` functions that are discussed in more detail in the [article on external, cached, and remote data sources](/articles/articles/extdata_cachedata.html).
