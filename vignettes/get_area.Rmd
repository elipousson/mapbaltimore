...
get_area(type = "council district",
...
area_dist_bbox <- sfx::st_extent(area_dist)

# buffer 1/4 (0.25) of the diagonal distance of the bounding box
area_diag_ratio <- buffer_area(area, diag_ratio = example_diag_ratio)
area_diag_ratio_bbox <- sfx::st_extent(area_diag_ratio)
...
area_dist_asp <- adjust_bbox_asp(area_dist, asp = example_asp) %>% 
  sfx::st_extent()

area_diag_ratio_asp <- adjust_bbox_asp(area_diag_ratio, asp = example_asp) %>% 
  sfx::st_extent()
...
get_area_data(area = buffer_area(area, diag_ratio = example_diag_ratio),
              data = council_districts,
              crop = FALSE)

get_area_data(bbox = sf::st_bbox(area),
              data = council_districts,
              diag_ratio = example_diag_ratio,
              crop = FALSE)
```
...
Like `ggplot2::geom_sf()`, `layer_area_data()` can inherit data from `ggplot()` and allows users to set aesthetics using the `mapping` parameter. Other ggplot2 objects such as scales, labels, and guides can be combined with a data layer by providing a list to the `layer_after` parameter. The `show_area` option also provides a convenient way to overlay the boundaries of the area and the `area_aes` parameter allows you to passed custom fixed aesthetics to the area layer. This following example showing the zoning categories around downtown illustrates how these parameters work.
...
layer_area_trees <- layer_area_data(
  area = area,
  extdata = "trees",
  .f = ~ dplyr::filter(.x, condition != "Dead"),
  trim = TRUE,
  mapping = aes(
    size = dbh * 0.4,
